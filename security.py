...
def authenticate(username, password):
    """
    Function that gets called when the user calls the /auth endpoint
    with their username and password.
    :param username: User's username in string format.
    :param password: User's un-encrypted password in stirng format.
...
def identity(payload):
    ...
    :param payload:A dictionary with 'identity' key, which has the user id.
